...
const CourseDashboardDisplay: React.FC<ICourseDashboardDisplayProps> = ({
  course,
}) => {
  ...
  return (
    <div className="course-display-container">
      <div className="course-display-title-container">
        <div className="course-display-title">{title}</div>
      </div>
        ...
        <div className="course-display-info-container">
          <div className="course-display-info">
            <div className="date-container">
              <div className="date-description">Start Date:</div>
              <div className="date-value">{startDate}</div>
            </div>
            <div className="date-container">
              <div className="date-description">End Date:</div>
              <div className="date-value">{endDate}</div>
            </div>
          </div>
          <UniButton custombutton="view" onClick={viewCourse}>
            {Date.parse(`${year}-${month}-${day}`) > Date.now() ||
            progress === 100
              ? `View course`
              : progress === 0
                ? `Start`
