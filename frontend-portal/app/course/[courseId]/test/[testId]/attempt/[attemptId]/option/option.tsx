...
const SingleOption: React.FC<SingleOptionProps> = ({ questionId, option }) => {
  ...
  const isSubmitedSelected =
    questionId in answers && answers[questionId].optionId === optionId;
  ...
  const selectOption = async () => {
    if (
      !(questionId in questionsAnswered) ||
      questionsAnswered[questionId] !== optionId
    ) {
      dispatch(
        updateQuestionAnswer({
      ...
      await AttemptRequest.saveQuestionAttempt({
