...
import {
  ITestResponseWithAttemptInfo,
  ScoringFormats,
  TestTypes,
} from "@/utils/request/types/test.types";
      ...
      <div className="single-test-quiz">
        ...
        <div className="quiz-status-with-button">
          <div className="quiz-status">
            {numOfAttempts === 0 ? `Not completed` : `Completed`}
          </div>
          {isEligible ? (
            <UniButton custombutton="confirm" onClick={clickLinkOrButton}>
              Attempt
            </UniButton>
          ) : (
            <div className="not-eligible-container">
              <a onClick={clickLinkOrButton}>
                <div className="test-detail-link">
                  <div className="test-detail">See details</div>
          ...
          {testType === TestTypes.EXAM ? (
            <div className="test-info">
              <div className="test-info-item">
                <div className="item-description">Scoring format:</div>
                <div className="item-value">
                  {renderScoringFormat(scoringFormat as ScoringFormats)}
                </div>
              </div>
              <div className="test-info-item">
                <div className="item-description">No. of attempts:</div>
                <div className="item-value">
                  {numOfAttempts}/{maxAttempt}
                </div>
              </div>
              <div className="test-info-item">
                <div className="item-description">Current score:</div>
                <div className="item-value">
                  {currScore ? `${currScore}/${maxScore}` : `--`}
                </div>
              </div>
            </div>
          ) : (
            <div className="test-info-item">
              <div className="item-description">Highest score:</div>
              <div className="item-value">
                {currScore ? `${currScore}/${maxScore}` : `--`}
              </div>
            </div>
