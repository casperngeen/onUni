...
const LoginPage: React.FC<{}> = () => {
    ...
    const submitLogin = async () => {
        ...
        if (!emailPattern.test(emailRef.current)) {
            ...
            try {
                const {refreshToken, accessToken} = await AuthRequest.login({email: emailRef.current, password: passwordRef.current}, dispatch);
                setError('');
                dispatch(login({refreshToken: refreshToken, accessToken: accessToken}))
                router.push(`/`);
            } catch (error) {
