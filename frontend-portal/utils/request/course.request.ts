...
export default class CourseRequest extends BaseRequest {
  public static async viewAllCourses() {
    return await BaseRequest.request<AllCourseResponseObject>(
      "course",
      RequestTypes.GET,
      {},
    );
  ...
  public static async viewCourse(params: IGetCourse) {
    ...
    return await BaseRequest.request<SingleCourseResponse>(
      `course/${courseId}`,
      RequestTypes.GET,
      {},
    );
  ...
  public static async createNewCourse(newCourse: NewCourseBody) {
    const data = await BaseRequest.request<NewCourseResponse>(
      "course",
      RequestTypes.POST,
      newCourse,
    );
