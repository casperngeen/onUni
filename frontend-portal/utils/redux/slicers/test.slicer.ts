...
const testSlice = createAppSlice({
  ...
  reducers: (create) => ({
    ...
    setCurrIndex: create.reducer((state, action: PayloadAction<number>) => {
      state.currIndex = state.testOrder.findIndex(
    ...
    getAllTests: create.asyncThunk.withTypes<{
    ...
    fetchTestInfo: create.asyncThunk.withTypes<{
      rejectValue: { message: string; errorCode: number };
    }>()(
      async (params: IGetTest, thunkAPI) => {
        try {
          let { deadline, start, attempts, ...testInfo } =
            await TestRequest.getTest(params);
          if (start) {
      ...
      },
      {
        ...
        fulfilled: (state, action) => {
          if (action.payload) {
            ...
            (state.start = start), (state.deadline = deadline);
