...
const attemptSlice = createAppSlice({
  ...
  initialState,
  reducers: (create) => ({
    updateQuestionAnswer: create.reducer(
      (state, action: PayloadAction<ISaveQuestionAttemptBody>) => {
        state.questionsAnswers[action.payload.questionId] =
    ...
    unbookmarkQuestion: create.reducer(
    ...
    setSubmitStatus: create.reducer(
      (state, action: PayloadAction<SubmitStatus>) => {
    ...
    submitAttempt: create.asyncThunk.withTypes<{
    ...
    deleteAttempt: create.asyncThunk.withTypes<{
      rejectValue: { message: string; errorCode: number };
    }>()(
    ...
    getAttempt: create.asyncThunk.withTypes<{
      rejectValue: { message: string; errorCode: number };
    }>()(
          ...
          if (action.payload) {
            ...
            if (action.payload.timeRemaining !== null) {
              if (action.payload.timeRemaining === 0) {
                ...
                state.timeRemaining =
                  Math.floor(action.payload.timeRemaining / 1000) + 1;
              }
            }
            if (
              action.payload.status === Status.SUBMIT ||
              action.payload.status === Status.AUTOSUBMIT
            ) {
              state.submitStatus = SubmitStatus.SUCCESS;
              for (let i = 0; i < action.payload.questionAttempts.length; i++) {
                const qAttempt = action.payload.questionAttempts[i];
                const isCorrect =
                  qAttempt.answerStatus === AnswerStatus.CORRECT;
                state.answers[qAttempt.questionId] = {
                  optionId: qAttempt.selectedOptionId,
                  isCorrect: isCorrect,
                };
              }
              ...
              const bookmark = localStorage.getItem(
                `bookmark-${action.payload.attemptId}`,
              );
              if (bookmark) {
              ...
              for (const qAttempt of action.payload.questionAttempts) {
                state.questionsAnswers[qAttempt.questionId] =
                  qAttempt.selectedOptionId;
              }
              state.readyForView = false;
            }
          }
