...
const courseSlice = createAppSlice({
  ...
  reducers: (create) => ({
    ...
    fetchCourseInfo: create.asyncThunk.withTypes<{
      rejectValue: { message: string; errorCode: number };
    }>()(
      async (params: IGetCourse, thunkAPI) => {
        try {
          let { startDate, endDate, title, description, tests } =
            await CourseRequest.viewCourse(params);
...
export const { fetchCourseInfo, resetError } = courseSlice.actions;
